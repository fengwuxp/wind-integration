 ...
 **/
@AllArgsConstructor
public class CompositeFreezeFundsAccountTransactionServiceImpl implements BasedOnFreezeFundsAccountTransactionService {

    private final Collection<BasedOnFreezeFundsAccountTransactionService> delegates;
    ...
    @Override
    public @NonNull String unfreeze(@NonNull FundsAccountId accountId, @NonNull FundsAccountTransactionRequest request) {
        return getDelegate(accountId).unfreeze(accountId, request);
    }

    @Override
    public @NonNull String freezeAndDebit(@NonNull FundsAccountId accountId, @NonNull FundsAccountTransactionRequest request) {
        return getDelegate(accountId).freezeAndDebit(accountId, request);
    }

    @Override
    public @NonNull String debitAvailableBalance(@NonNull FundsAccountId accountId, @NonNull FundsAccountTransactionRequest request) {
        return getDelegate(accountId).debitAvailableBalance(accountId, request);
    }
    ...
    private BasedOnFreezeFundsAccountTransactionService getDelegate(FundsAccountId id) {
        Optional<BasedOnFreezeFundsAccountTransactionService> result = delegates.stream().filter(delegate -> delegate.supports(id)).findFirst();
