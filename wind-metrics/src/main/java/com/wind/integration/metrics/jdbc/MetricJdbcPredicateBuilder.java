...
/**
 * 指标 JDBC 谓词构造器，将指标 DSL 过滤条件构造为 jOOQ 条件表达式。
 *
 * <h2>核心职责</h2>
 * <ul>
 *   <li>条件构造：将 {@link MetricFilterDsl} 递归转换为 jOOQ {@link Condition}</li>
 ...
 *   <li><b>逻辑运算</b>：AND, OR（递归处理已校验的嵌套条件）</li>
 * </ul>
 *
 * <h2>使用场景</h2>
 * <p>被事实查询和有限行集查询共用：</p>
 ...
 *
 * <h2>设计约束</h2>
 * <p>本类保留在包内，产出条件对象并收集参数；最终 SQL 文本及占位符顺序由
 * {@link MetricJdbcSqlCompiler} 生成。表路由、数据源选择和分库分表由宿主负责。</p>
 *
 * @author wuxp
 */
final class MetricJdbcPredicateBuilder {

    private final MetricJdbcValueNormalizer normalizer;

    MetricJdbcPredicateBuilder(MetricJdbcValueNormalizer normalizer) {
        this.normalizer = normalizer;
    }

    Condition build(MetricJdbcMapping binding, MetricFilterDsl filter,
                     Map<String, MetricJdbcParameterBinding> parameters, Function<String, Field<Object>> columns) {
        ...
        for (MetricFilterDsl operand : logical.operands()) {
            operands.add(build(binding, operand, parameters, columns));
        }
