...
/**
 * 一条快照记录的保存请求，携带已累计完成的指标对象、业务身份和拟提交的累计覆盖。
 *
 ...
 *
 * <h2>保存内容与职责边界</h2>
 * <p>snapshotValue 包含 T 所声明的完整信息，可以只表示一个指标或字段，不要求包含整份计划的指标；
 * 仓储不能再次叠加旧值。需要共同提交的多个记录由仓储实现识别和归组。
 * 请求中的主体、完整维度和覆盖是本次保存上下文的权威输入；业务对象已有的同义元数据须与其一致，
 * 不能另成一套独立输入。版本及目标身份等宿主元数据按具体对象和仓储实现约定处理。</p>
 * <p>请求只传原始 dimensionValues，不要求调用方同时传入派生的存储维度键。
 * 仓储或所属基础服务按本次确定的定义与目标生成存储键，并保持与读取侧相同的规范化规则。
 * T 可以直接对应一行，也可以由实现拆成多行；同一保存组需要共同提交的值、覆盖和版本须原子保存。</p>
 *
 * <h2>累计覆盖与水位</h2>
 ...
 *
 * @param snapshotValue 已累计完成的快照记录，包含 T 声明的值及所需身份、版本信息
